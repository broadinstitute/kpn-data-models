...
4. **Attach trait release files** uploads the five versioned exports as release assets. **Publish latest release to Pages** independently resolves GitHub's latest stable published release, checks out that tag, and builds from `versions/trait/<tag>/`. No dependency on asset-upload timing is needed.
