...
- `kpn_trait_collection.yaml` — complete phenotype collection.
- `kpn_trait_flat.tsv` — one row per mapping with phenotype details.
- `kpn_trait_registry.tsv` — stable IDs and phenotype names.
...
`tests/fixtures/v0.0.1-baseline.json` records content hashes from commit `dcdb13b9ed8e39007678d8b488dc3a7197f2096b`, before the prefix migration. Tests normalize only the identifier prefix, the core source category rename (including its two coverage-report labels), and SSSOM comment metadata. Every numeric ID, other phenotype field, mapping, primary selection, row order, and coverage statistic must remain identical. Separate checks enforce the new namespace and source categories, declared CURIE prefixes, and byte-for-byte offline regeneration. Do not refresh that fixture to hide a regression; intentional mapping changes belong in the next version.
