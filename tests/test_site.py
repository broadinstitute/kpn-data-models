...
class SiteTests(unittest.TestCase):
    ...
    def test_direct_pages_complete_record_provenance_and_escaping(self):
        ...
        self.assertEqual(index[0]['source'], 'KPN')
        catalogue = (self.output / 'index.html').read_text()
        self.assertIn('<option>KPN</option>', catalogue)
        self.assertNotIn('<option>portal</option>', catalogue)
